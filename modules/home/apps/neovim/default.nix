...
let
  ...
  map = mapping: lib.flatten (lib.mapAttrsToList
    ...
    programs.nixvim = enabled' {
      ...
      keymaps = map {
        n = {
          ...
          "<C-c>" = { action = "<cmd>%y+<cr>"; options.silent = true; };
          "<Tab>" = { action = "<cmd>bn<cr>"; options.silent = true; };
          "<S-Tab>" = { action = "<cmd>bp<cr>"; options.silent = true; };
